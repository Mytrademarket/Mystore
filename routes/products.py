...
def build_product_grid(products):
    ...
    for p in products:
        ...
        if image and "http" in image:
            ...
            img_src = f"/static/uploads/{image}"
        ...
        <div class="col-md-3 mb-4">
            <div class="card h-100 shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <a href="/product/{p['id']}" class="btn btn-dark btn-sm w-100">
                    View Product
                    </a>
...
def home():
    ...
    sliders = conn.execute(
        "SELECT * FROM sliders WHERE active=1"
    ).fetchall()
    ...
    hot = conn.execute(
    "SELECT * FROM products ORDER BY stock DESC LIMIT 8"
).fetchall()
    ...
    homepage = conn.execute(
        "SELECT value FROM site_settings WHERE key='homepage_content'"
    ).fetchone()

    whatsapp = conn.execute(
        "SELECT value FROM site_settings WHERE key='whatsapp'"
    ).fetchone()
...
def delete_product_image(image_id):
    ...
    if image:
        path = os.path.join(current_app.root_path, "static/uploads", image["image"])
